...
<script>
	...
	export default {
		...
		methods: {
			getData() {
				floorFindProduct(this.$route.query.id).then(res => {
					this.list = res.data;
				})
			},
			getList() {
				...
				floorPageProduct(this.$route.query.id, data).then(res => {
			...
			handleSelect() {
				this.$confirm('确定是否要将当前分页选择的商品添加到这个分类下', '添加提示', {
					...
					floorSaveProduct(this.$route.query.id, ids).then(res =>{
						this.$message({
							message: '添加成功',
							type: 'success'
						});
			...
			handleCloseTag(item, index){
				this.$confirm('确定要把这个商品从这个分类下删除吗？', '删除提示', {
					...
					floorDeleteProduct(this.$route.query.id, item.id).then(res =>{
						this.$message({
							message: '删除成功',
							type: 'success'
						});
