							...
							<el-upload class="avatar-uploader" action="/api/v1/img/save" :show-file-list="false" :on-success="handleAvatarSuccess"
							 :before-upload="beforeAvatarUpload">
								...
								<div v-if="topic.image" class="el-upload-list--picture-card image-box">
									<img style="object-fit: contain;" class="el-upload-list__item-thumbnail" :src="topic.image" alt="">
...
<script>
	...
	export default {
		...
		methods: {
			getData(){
				detailTopic(this.$route.query.id).then(res =>{
					this.topic = res.data;
				})
			},
			submitForm() {
				this.$refs['ruleForm'].validate((valid) => {
					if (valid) {
						updateTopic(this.topic).then(res =>{
							this.$message({
								message: '编辑成功',
								type: 'success'
							});
