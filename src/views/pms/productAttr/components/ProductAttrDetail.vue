	...
	<el-card class="form-container" shadow="never">
		<el-form :model="productAttr" :rules="rules" ref="productAttrFrom" label-width="150px">
			...
			<el-form-item label="商品类型：">
				<div>{{productAttributeCategoryName}}</div>
			</el-form-item>
			<el-form-item label="是否是颜色属性:" v-if="$route.query.type == 0">
				<el-radio-group v-model="productAttr.isColor">
...
<script>
	...
	export default {
		...
		created() {
			...
			this.getProductAttributeCategoryName();
		},
		watch: {
		...
		methods: {
			getProductAttributeCategoryName() {
				detailProductAttrCate(this.$route.query.cid).then(res =>{
					this.productAttributeCategoryName = res.data.name;
				})
			},
			resetProductAttr() {
					...
					if (valid) {
						this.$confirm('是否提交数据', '提示', {
							...
							if(this.$route.query.type == 1){
								delete this.productAttr.isColor;
							}
							if (this.isEdit) {
