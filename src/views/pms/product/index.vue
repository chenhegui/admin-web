<template> 
	<div class="app-container">
			...
			<div style="margin-top: 15px">
				<el-form :inline="true" :model="listQuery" size="small" label-width="140px">
					<el-form-item label="输入搜索：">
						<el-input style="width: 200px" v-model="listQuery.keyword" placeholder="商品名称" clearable></el-input>
					</el-form-item>
		...
		<div class="table-container">
            ...
            <img style="width: 140px;height: 140px;"  :src="scope.row.pic" />
						<!-- <el-image style="width: 140px;height: 140px;" :src="scope.row.pic" :preview-src-list="[scope.row.pic]"> -->
						<!-- </el-image> -->
					</template>
				</el-table-column>
				...
				<el-table-column label="标签" width="140" align="center">
					<template slot-scope="scope">
						...
						<p>推荐：
							<el-switch @change="handleRecommendStatusChange(scope.$index, scope.row)" :active-value="1" :inactive-value="0"
		...
		<div class="pagination-container">
			<el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange" layout="total, sizes,prev, pager, next,jumper"
			 :page-size="listQuery.pageSize" :page-sizes="[5,10,15]" :current-page.sync="listQuery.pageIndex" :total="total">
			</el-pagination>
		</div>
	...
	export default {
		...
		methods: {
			getProductSkuSp(row, index) {
				return row.sp.split(',')[index];
			},
			getList() {
				this.listLoading = true;
				pageProduct(this.listQuery).then(response => {
			...
			handleShowSkuEditDialog(index, row) {
				...
				listSkuStock(row.id, { keyword: this.editSkuInfo.keyword }).then(response => {
					this.editSkuInfo.stockList = response.data;
				});
				if (row.productAttributeCategoryId != null) {
					listProductAttr(row.productAttributeCategoryId, { type: 0 }).then(response => {
						this.editSkuInfo.productAttr = response.data;
					});
				}
			},
			handleSearchEditSku() {
				listSkuStock(this.editSkuInfo.productId, { keyword: this.editSkuInfo.keyword }).then(response => {
					this.editSkuInfo.stockList = response.data;
				});
			},
			handleEditSkuConfirm() {
				...
				this.$confirm('是否要进行修改', '提示', {
					...
					updateSkuStock(this.editSkuInfo.productId, this.editSkuInfo.stockList).then(response => {
						this.$message({
							message: '修改成功',
							type: 'success',
							duration: 1000
